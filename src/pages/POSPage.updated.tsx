...
import {
  ...
  ShoppingBag, Printer, X, CheckCircle, RotateCcw,
} from 'lucide-react';
...
export default function POSPage() {
  ...
  // Currency scan state khusus pembayaran tunai
  ...
  const handleScanMoney = async () => {
    ...
    try {
      ...
      if (result.is_authentic && detectedNominal > 0) {
        setCashInput(String(detectedNominal));
      }
    } catch (error: any) {
      console.error(error);
  ...
  // ── Validation helpers ────────────────────────────────────────────────────
  const isCashValid = payMethod !== 'cash' || (!!cashInput && Number(cashInput) >= total);
  const isCardValid = (payMethod !== 'debit' && payMethod !== 'credit') || !!cardNumber;
  const isMoneyScanValid =
    payMethod !== 'cash' ||
    (!!scanResult && scanResult.is_authentic && scanResult.nominal_detected);
  ...
  const handlePayment = async () => {
    if (!canConfirm) {
      ...
      if (!isCashValid) { alert('Uang tidak cukup!'); return; }
      if (!isMoneyScanValid) { alert('Scan uang tunai terlebih dahulu dan pastikan uang asli serta nominal terdeteksi'); return; }
                ...
                <div>
                  <div style={{
                    ...
                    <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
                      <Banknote size={15} /> Scan Uang Tunai
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 10 }}>
                      Upload foto uang untuk mendeteksi nominal dan status asli/palsu sebelum transaksi dikonfirmasi.
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      >
                        {isScanning ? 'Scanning...' : 'Scan Uang'}
                      </button>
                      {(moneyImage || scanResult) && (
                        <button
                          className="btn btn-secondary"
                          onClick={clearMoneyScan}
                    ...
                    {scanResult && (
                      <div style={{
                        ...
                        {!scanResult.is_authentic && (
                          <div style={{ marginTop: 8, color: '#f87171', fontWeight: 600 }}>
                            Uang terdeteksi palsu. Pembayaran tunai tidak bisa dikonfirmasi.
                          </div>
                        )}
                        {scanResult.is_authentic && !scanResult.nominal_detected && (
                          <div style={{ marginTop: 8, color: '#f59e0b', fontWeight: 600 }}>
                            Uang asli, tetapi nominal belum terdeteksi. Coba foto ulang dengan pencahayaan lebih jelas.
                          </div>
                        )}
                      </div>
                    )}
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
                    {quickCash.map((v, i) => (
                      <button key={i} onClick={() => setCashInput(String(v))} className="btn btn-secondary btn-sm">
                        {formatCurrency(v)}
                      </button>
                    ))}
                  </div>
